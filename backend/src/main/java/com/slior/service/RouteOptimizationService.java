...
public class RouteOptimizationService {
    ...
    public RouteResponse optimizarRuta(UUID routeId, OptimizeRouteRequest request) {
        ...
        // 3. Calcular distancia total y tiempo estimado basado en tipo de vehículo
        ...
        // Calcular tiempo estimado usando la velocidad del vehículo
        VehicleType vehicleType = request.vehicleType();
        int tiempoEstimadoMinutos = calcularTiempoEstimado(distanciaKm, vehicleType);
        route.setTiempoEstimado(tiempoEstimadoMinutos);

        return RouteResponse.from(routeRepository.save(route));
    }

    /**
     * Calcula el tiempo estimado en minutos basado en la distancia y el tipo de vehículo.
     * Los vehículos más lentos pueden tomar rutas más complejas pero optimizadas para su velocidad.
     */
    private int calcularTiempoEstimado(double distanciaKm, VehicleType vehicleType) {
        int tiempoBase = (int) Math.ceil((distanciaKm / vehicleType.getAverageSpeed()) * 60);
        // El multiplicador ajusta el tiempo según factores adicionales (tráfico, paradas, etc.)
        return (int) Math.ceil(tiempoBase * vehicleType.getTimeMultiplier());
    }
